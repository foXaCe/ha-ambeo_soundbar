...
class AmbeoEspressoApi(AmbeoApi):
    capabilities = [
        ...
        Capability.AMBEO_MODE,
        Capability.NIGHT_MODE,
        Capability.SOUND_FEEDBACK,
    ]
    ...
    async def get_sound_feedback(self):
        return await self.get_value(
            "settings:/espresso/soundFeedback", "bool_", "value"
        )
    ...
    async def get_all_sources(self):
        input_names_res = await self.execute_request(
            "getRows", "settings:/espresso/inputNames", "@all", None, 0, 20
        )
        inputs_res = await self.execute_request(
            "getRows", "espresso:", "@all", None, 0, 20
        )
    ...
    async def get_center_speaker_level(self):
        return await self.get_value(
            "ui:/settings/audio/centerSettings", "i16_", "value"
        )
    ...
    async def get_up_firing_level(self):
        return await self.get_value(
            "ui:/settings/audio/heightSettings", "i16_", "value"
        )
